...
You could indeed just do your changes on _main_, however creating branches is the proper _Git_ workflow, and it keeps your changes organized and separated from the _main_ branch so you can submit multiple pull requests with ease.
...
or without _--name-only_ if you want to go though all the changes (`Q` key to quit).
...
so it will open the assigned text editor. Write the main short message at the very start, and uncomment any other part of the sample you may want to add.
...
### Case C: Push directly on origin/main, as a owner/collaborator
If you are the owner, you can push directly to _main_.
...
If it is specified already but the name of the branches are different, i.e. you are on _my_branch_ but you want to push on _origin/main_:
...
`git log my_branch ^main --no-merges`
