...
* Naming conventions for repositories and branches:
  * `main`, historically `master`: the main branch of the repository.
  * `origin`: your fork.
...
After cloning a repository, it generally automatically creates a _main_ branch that tracks _origin/main_; however you can switch to and create other branches.
* `fast-forwarding`: what happens when you update your local repository with `git pull` and you get no issues; what it technically is, is merging two commits in one go, when there is no divergent work to merge together (and therefore no `merge commits`).
...
`main`: a branch which refers to commit `c`
...
`main` has been updated, and so has `HEAD`, which always points to `main`.
